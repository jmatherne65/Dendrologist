...
public class AVLTree<E extends Comparable<E>> implements AVLTreeAPI<E> {
   ...
   public boolean isPerfect() {
      double j = height(root) + 1;
      if (root == null){
         return true;
      } else
      if (Math.pow(2,j) - 1 == size())  {
         return true;
    ...
    * by exploring only nodes along the longest path in the tree using
    * the balance factors
    *
   ...
   private int height(Node node) {
    ...
    while (node != null){
       if (node.bal == BalancedFactor.EH){
          node = node.left;
          counter++;
       } else if (node.bal == BalancedFactor.LH){
          node = node.left;
          counter++;
       } else if (node.bal == BalancedFactor.RH){
          node = node.right;
          counter++;
       }
    }
     ...
     * of leaf/terminal nodes in this tree
     * @param node the root of a subtree
     * @return the number of leaf node in the subtree rooted at the 
     * specified node
     */
